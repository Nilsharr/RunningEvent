...
import { getEvents, deleteEvent, joinEvent, leaveEvent } from "../actions/events";
...
const EventsList = () => {
    ...
    const joinCurrentEvent = () => {
        dispatch(joinEvent(currentEvent.id))
            .then(() => {
                        ...
                        {user.admin ? (
                            <div className="mt-3">
                                <button className="btn btn-warning mr-2" type="button"
                                    onClick={() => window.location.assign(`/events/${currentEvent.id}`)}>
                                    Edit
                                </button>
                            ...
                            currentEvent.participants.includes(user.userId) ?
                                <div className="mt-3" >
                                    <button className="btn btn-danger" type="button"
